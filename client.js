try {
	...
	let props = Properties.GetContext(), state = props.Get("state"), main_timer = Timers.GetContext().Get("main"), update_timer = Timers.GetContext().Get("upd"),
		banned_id = props.Get("banned_id"), silver = AreaPlayerTriggerService.Get("silver"), gold = AreaPlayerTriggerService.Get("gold"), capture_blue = AreaPlayerTriggerService.Get("capture_blue"), capture_red = AreaPlayerTriggerService.Get("capture_red");
	...
	Teams.OnRequestJoinTeam.Add(function (p, t) {
		...
		if (p.Team == null) {
			if (props.Get(p.Id + "save").Value) {
				...
				p.contextedProperties.MaxHp.Value = DEFAULT_PROPS.Values[6];
			}
